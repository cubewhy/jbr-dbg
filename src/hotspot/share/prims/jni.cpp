...
#define DT_RETURN_MARK_DECL(name, type, probe)                                 \
  DTRACE_ONLY(class DTraceReturnProbeMark_##name {                             \
  public:                                                                      \
    const type &_ret_ref;                                                      \
    DTraceReturnProbeMark_##name(const type &v) : _ret_ref(v) {}               \
    ~DTraceReturnProbeMark_##name() { probe; }                                 \
  })
...
#define DT_RETURN_MARK_DECL_FOR(TypeName, name, type, probe)                   \
  FP_SELECT(TypeName, DT_RETURN_MARK_DECL(name, type, probe),                  \
            DT_VOID_RETURN_MARK_DECL(name, probe))
#define DT_RETURN_MARK_FOR(TypeName, name, type, ref)                          \
  FP_SELECT(TypeName, DT_RETURN_MARK(name, type, ref),                         \
            DT_VOID_RETURN_MARK(name))
...
void jfieldIDWorkaround::verify_instance_jfieldID(Klass *k, jfieldID id) {
  guarantee(jfieldIDWorkaround::is_instance_jfieldID(k, id),
  ...
  guarantee(
      InstanceKlass::cast(k)->contains_field_offset(offset),
...
JNI_ENTRY(jclass,
          jni_DefineClass(JNIEnv *env, const char *name, jobject loaderRef,
                          const jbyte *buf, jsize bufLen))
HOTSPOT_JNI_DEFINECLASS_ENTRY(env, (char *)name, loaderRef, (char *)buf,
                              bufLen);

jclass cls = nullptr;
DT_RETURN_MARK(DefineClass, jclass, (const jclass &)cls);

// Class resolution will get the class name from the .class stream if the name
// is null.
TempNewSymbol class_name =
    name == nullptr
        ? nullptr
        : SystemDictionary::class_name_symbol(
              name, vmSymbols::java_lang_NoClassDefFoundError(), CHECK_NULL);
...
Klass *k = SystemDictionary::resolve_from_stream(
    &st, class_name, class_loader, cl_info, nullptr, nullptr, CHECK_NULL);
...
TempNewSymbol class_name = SystemDictionary::class_name_symbol(
    name, vmSymbols::java_lang_NoClassDefFoundError(), CHECK_NULL);
...
if (k != nullptr) {
  ...
  if (k->class_loader() == nullptr &&
    ...
    JavaCalls::call_static(&result, k, vmSymbols::getFromClass_name(),
                           vmSymbols::void_class_signature(), CHECK_NULL);
...
result =
    find_class_from_class_loader(env, class_name, true, loader, true, thread);

if (log_is_enabled(Debug, class, resolve) && result != nullptr) {
  trace_class_resolution(
      java_lang_Class::as_Klass(JNIHandles::resolve_non_null(result)));
}
...
ret = m == nullptr ? nullptr
                   : m->jmethod_id(); // return null if reflected method deleted
return ret;
JNI_END
...
if (modifiers & JVM_ACC_STATIC) {
  ...
  // A jfieldID for a static field is a JNIid specifying the field holder and
  // the offset within the Klass*
...
int offset = InstanceKlass::cast(k1)->field_offset(slot);
assert(InstanceKlass::cast(k1)->contains_field_offset(offset),
       "stay within object");
...
JNI_ENTRY(jobject,
          jni_ToReflectedMethod(JNIEnv *env, jclass cls, jmethodID method_id,
                                jboolean isStatic))
...
assert(m->is_static() == (isStatic != 0),
       "jni_ToReflectedMethod access flags doesn't match");
...
DEBUG_ONLY(Klass *super2 = (k->is_array_klass() ? vmClasses::Object_klass()
                                                : k->super()));
assert(super == super2,
       "java_super computation depends on interface, array, other super");
obj = (super == nullptr)
          ? nullptr
          : (jclass)JNIHandles::make_local(THREAD, super->java_mirror());
return obj;
JNI_END

JNI_ENTRY_NO_PRESERVE(jboolean, jni_IsAssignableFrom(JNIEnv *env, jclass sub,
                                                     jclass super))
...
assert(sub_klass != nullptr && super_klass != nullptr,
       "invalid arguments to jni_IsAssignableFrom");
...
InstanceKlass *k = InstanceKlass::cast(
...
if (thread->has_pending_exception()) {
  ...
  if (thread != nullptr && thread->threadObj() != nullptr) {
    ResourceMark rm(THREAD);
    jio_fprintf(defaultStream::error_stream(), "in thread \"%s\" ",
                thread->name());
  }
  if (ex->is_a(vmClasses::Throwable_klass())) {
    JavaValue result(T_VOID);
    JavaCalls::call_virtual(&result, ex, vmClasses::Throwable_klass(),
                            vmSymbols::printStackTrace_name(),
                            vmSymbols::void_method_signature(), THREAD);
    // If an exception is thrown in the call it gets thrown away. Not much
    // we can do with it. The native code that calls this, does not check
    // for the exception - hence, it might still be in the thread when DestroyVM
    // gets called, potentially causing a few asserts to trigger - since no
    // pending exception is expected.
...
if (new_handles != nullptr) {
  // As a sanity check we only release the handle blocks if the pop_frame_link
  // is not null. This way code will still work if PopLocalFrame is called
  // without a corresponding PushLocalFrame call. Note that we set the
  // pop_frame_link to null explicitly, otherwise the release_block call will
  // release the blocks.
  thread->set_active_handles(new_handles);
  old_handles->set_pop_frame_link(
      nullptr); // clear link we won't release new_handles below
...
jobject ret =
    JNIHandles::make_global(ref_handle, AllocFailStrategy::RETURN_NULL);
...
JNI_ENTRY_NO_PRESERVE(jboolean,
                      jni_IsSameObject(JNIEnv *env, jobject r1, jobject r2))
...
protected:
  ...
  void push_boolean(jboolean b) {
    // Normalize boolean arguments from native code by converting 1-255 to
    // JNI_TRUE and 0 to JNI_FALSE.  Boolean return values from native are
    // normalized the same in
  ...
  JNI_ArgumentPusher(Method *method)
      : SignatureIterator(method->signature(),
                          Fingerprinter(methodHandle(Thread::current(), method))
                              .fingerprint()) {
    _arguments = nullptr;
  }
...
class JNI_ArgumentPusherVaArg : public JNI_ArgumentPusher {
  ...
  void set_ap(va_list rap) { va_copy(_ap, rap); }
  ...
  void do_type(BasicType type) {
    ...
    case T_BOOLEAN:
      ...
      // each of these paths is exercised by the various jck
      // Call[Static,Nonvirtual,][Void,Int,..]Method[A,V,] tests
...
public:
  ...
  ~JNI_ArgumentPusherVaArg() { va_end(_ap); }
...
enum JNICallType { JNI_STATIC, JNI_VIRTUAL, JNI_NONVIRTUAL };

static void jni_invoke_static(JNIEnv *env, JavaValue *result, jobject receiver,
                              JNICallType call_type, jmethodID method_id,
                              JNI_ArgumentPusher *args, TRAPS) {
...
static void jni_invoke_nonstatic(JNIEnv *env, JavaValue *result,
  ...
  Method *selected_method;
  {
    ...
    if (call_type != JNI_VIRTUAL) {
      ...
      selected_method =
          InstanceKlass::cast(k)->method_at_itable(holder, itbl_index, CHECK);
    }
  }
  ...
  // handle arguments
  assert(!method->is_static(), "method %s should not be static",
...
instanceOop i = InstanceKlass::allocate_instance(
    JNIHandles::resolve_non_null(clazz), CHECK_NULL);
...
JNI_ENTRY(jobject, jni_NewObjectA(JNIEnv *env, jclass clazz, jmethodID methodID,
                                  const jvalue *args))
...
jni_invoke_nonstatic(env, &jvalue, obj, JNI_NONVIRTUAL, methodID, &ap,
                     CHECK_NULL);
return obj;
JNI_END
...
JNI_ENTRY(jobject, jni_NewObjectV(JNIEnv *env, jclass clazz, jmethodID methodID,
                                  va_list args))
...
jclass ret = (jclass)JNIHandles::make_local(THREAD, k->java_mirror());
...
JNI_ENTRY_NO_PRESERVE(jboolean,
                      jni_IsInstanceOf(JNIEnv *env, jobject obj, jclass clazz))
...
static jmethodID get_method_id(JNIEnv *env, jclass clazz, const char *name_str,
  ...
  const char *name_to_probe =
      (name_str == nullptr)
          ? vmSymbols::object_initializer_name()->as_C_string()
          : name_str;
  TempNewSymbol name =
      SymbolTable::probe(name_to_probe, (int)strlen(name_to_probe));
  ...
  if (java_lang_Class::is_primitive(mirror)) {
    ...
    THROW_MSG_NULL(vmSymbols::java_lang_NoSuchMethodError(),
                   err_msg("%s%s.%s%s", is_static ? "static " : "",
                           klass->signature_name(), name_str, sig));
  }
  ...
  if (name == vmSymbols::object_initializer_name() ||
    ...
    if (m == nullptr && klass->is_instance_klass()) {
      m = InstanceKlass::cast(klass)->lookup_method_in_ordered_interfaces(
          name, signature);
    }
  }
...
JNI_ENTRY(jmethodID, jni_GetStaticMethodID(JNIEnv *env, jclass clazz,
                                           const char *name, const char *sig))
HOTSPOT_JNI_GETSTATICMETHODID_ENTRY(env, (char *)clazz, (char *)name,
                                    (char *)sig);
...
#define DEFINE_CALLMETHOD(ResultType, Result, Tag, EntryProbe, ReturnProbe)    \
  ...
  DT_RETURN_MARK_DECL_FOR(Result, Call##Result##Method, ResultType,            \
                          ReturnProbe);                                        \
                                                                               \
  JNI_ENTRY(ResultType, jni_Call##Result##Method(JNIEnv *env, jobject obj,     \
                                                 jmethodID methodID, ...))     \
                                                                               \
  EntryProbe;                                                                  \
  ResultType ret{};                                                            \
...
#define DEFINE_CALLMETHODA(ResultType, Result, Tag, EntryProbe, ReturnProbe)   \
  ...
  DT_RETURN_MARK_DECL_FOR(Result, Call##Result##MethodA, ResultType,           \
                          ReturnProbe);                                        \
                                                                               \
  JNI_ENTRY(ResultType,                                                        \
            jni_Call##Result##MethodA(JNIEnv *env, jobject obj,                \
...
JNI_ENTRY(void,
          jni_CallVoidMethod(JNIEnv *env, jobject obj, jmethodID methodID, ...))
...
JNI_ENTRY(void, jni_CallVoidMethodV(JNIEnv *env, jobject obj,
                                    jmethodID methodID, va_list args))
...
#define DEFINE_CALLNONVIRTUALMETHOD(ResultType, Result, Tag, EntryProbe,       \
                                    ReturnProbe)                               \
                                                                               \
  DT_RETURN_MARK_DECL_FOR(Result, CallNonvirtual##Result##Method, ResultType,  \
                          ReturnProbe);                                        \
                                                                               \
  JNI_ENTRY(ResultType, jni_CallNonvirtual##Result##Method(                    \
...
DEFINE_CALLNONVIRTUALMETHOD(
    ...
    HOTSPOT_JNI_CALLNONVIRTUALBOOLEANMETHOD_ENTRY(env, obj, cls,
                                                  (uintptr_t)methodID),
    HOTSPOT_JNI_CALLNONVIRTUALBOOLEANMETHOD_RETURN(_ret_ref))
DEFINE_CALLNONVIRTUALMETHOD(
    jbyte, Byte, T_BYTE,
    HOTSPOT_JNI_CALLNONVIRTUALBYTEMETHOD_ENTRY(env, obj, cls,
                                               (uintptr_t)methodID),
    HOTSPOT_JNI_CALLNONVIRTUALBYTEMETHOD_RETURN(_ret_ref))
DEFINE_CALLNONVIRTUALMETHOD(
    jchar, Char, T_CHAR,
    HOTSPOT_JNI_CALLNONVIRTUALCHARMETHOD_ENTRY(env, obj, cls,
                                               (uintptr_t)methodID),
    HOTSPOT_JNI_CALLNONVIRTUALCHARMETHOD_RETURN(_ret_ref))
DEFINE_CALLNONVIRTUALMETHOD(
    jshort, Short, T_SHORT,
...
DEFINE_CALLNONVIRTUALMETHOD(
    ...
    HOTSPOT_JNI_CALLNONVIRTUALOBJECTMETHOD_ENTRY(env, obj, cls,
                                                 (uintptr_t)methodID),
    HOTSPOT_JNI_CALLNONVIRTUALOBJECTMETHOD_RETURN(_ret_ref))
DEFINE_CALLNONVIRTUALMETHOD(
    jint, Int, T_INT,
    HOTSPOT_JNI_CALLNONVIRTUALINTMETHOD_ENTRY(env, obj, cls,
                                              (uintptr_t)methodID),
...
DEFINE_CALLNONVIRTUALMETHOD(
    ...
    HOTSPOT_JNI_CALLNONVIRTUALLONGMETHOD_ENTRY(env, obj, cls,
                                               (uintptr_t)methodID),
    // Float and double probes don't return value because dtrace doesn't
...
#define DEFINE_CALLNONVIRTUALMETHODV(ResultType, Result, Tag, EntryProbe,      \
                                     ReturnProbe)                              \
                                                                               \
  DT_RETURN_MARK_DECL_FOR(Result, CallNonvirtual##Result##MethodV, ResultType, \
                          ReturnProbe);                                        \
                                                                               \
  JNI_ENTRY(ResultType, jni_CallNonvirtual##Result##MethodV(                   \
                            JNIEnv *env, jobject obj, jclass cls,              \
                            jmethodID methodID, va_list args))                 \
                                                                               \
  EntryProbe;                                                                  \
...
DEFINE_CALLNONVIRTUALMETHODV(
    jboolean, Boolean, T_BOOLEAN,
    HOTSPOT_JNI_CALLNONVIRTUALBOOLEANMETHODV_ENTRY(env, obj, cls,
                                                   (uintptr_t)methodID),
    HOTSPOT_JNI_CALLNONVIRTUALBOOLEANMETHODV_RETURN(_ret_ref))
DEFINE_CALLNONVIRTUALMETHODV(
    jbyte, Byte, T_BYTE,
    HOTSPOT_JNI_CALLNONVIRTUALBYTEMETHODV_ENTRY(env, obj, cls,
                                                (uintptr_t)methodID),
    HOTSPOT_JNI_CALLNONVIRTUALBYTEMETHODV_RETURN(_ret_ref))
DEFINE_CALLNONVIRTUALMETHODV(
    jchar, Char, T_CHAR,
    HOTSPOT_JNI_CALLNONVIRTUALCHARMETHODV_ENTRY(env, obj, cls,
                                                (uintptr_t)methodID),
...
DEFINE_CALLNONVIRTUALMETHODV(
    jobject, Object, T_OBJECT,
    HOTSPOT_JNI_CALLNONVIRTUALOBJECTMETHODV_ENTRY(env, obj, cls,
                                                  (uintptr_t)methodID),
    HOTSPOT_JNI_CALLNONVIRTUALOBJECTMETHODV_RETURN(_ret_ref))
DEFINE_CALLNONVIRTUALMETHODV(
    jint, Int, T_INT,
    HOTSPOT_JNI_CALLNONVIRTUALINTMETHODV_ENTRY(env, obj, cls,
                                               (uintptr_t)methodID),
...
#define DEFINE_CALLNONVIRTUALMETHODA(ResultType, Result, Tag, EntryProbe,      \
                                     ReturnProbe)                              \
                                                                               \
  DT_RETURN_MARK_DECL_FOR(Result, CallNonvirtual##Result##MethodA, ResultType, \
                          ReturnProbe);                                        \
                                                                               \
  JNI_ENTRY(ResultType, jni_CallNonvirtual##Result##MethodA(                   \
                            JNIEnv *env, jobject obj, jclass cls,              \
                            jmethodID methodID, const jvalue *args))           \
                                                                               \
  EntryProbe;                                                                  \
...
DEFINE_CALLNONVIRTUALMETHODA(
    jboolean, Boolean, T_BOOLEAN,
    HOTSPOT_JNI_CALLNONVIRTUALBOOLEANMETHODA_ENTRY(env, obj, cls,
                                                   (uintptr_t)methodID),
    HOTSPOT_JNI_CALLNONVIRTUALBOOLEANMETHODA_RETURN(_ret_ref))
DEFINE_CALLNONVIRTUALMETHODA(
    jbyte, Byte, T_BYTE,
    HOTSPOT_JNI_CALLNONVIRTUALBYTEMETHODA_ENTRY(env, obj, cls,
                                                (uintptr_t)methodID),
    HOTSPOT_JNI_CALLNONVIRTUALBYTEMETHODA_RETURN(_ret_ref))
DEFINE_CALLNONVIRTUALMETHODA(
    jchar, Char, T_CHAR,
    HOTSPOT_JNI_CALLNONVIRTUALCHARMETHODA_ENTRY(env, obj, cls,
                                                (uintptr_t)methodID),
...
DEFINE_CALLNONVIRTUALMETHODA(
    jobject, Object, T_OBJECT,
    HOTSPOT_JNI_CALLNONVIRTUALOBJECTMETHODA_ENTRY(env, obj, cls,
                                                  (uintptr_t)methodID),
    HOTSPOT_JNI_CALLNONVIRTUALOBJECTMETHODA_RETURN(_ret_ref))
DEFINE_CALLNONVIRTUALMETHODA(
    jint, Int, T_INT,
    HOTSPOT_JNI_CALLNONVIRTUALINTMETHODA_ENTRY(env, obj, cls,
                                               (uintptr_t)methodID),
...
JNI_ENTRY(void,
          jni_CallNonvirtualVoidMethod(JNIEnv *env, jobject obj, jclass cls,
                                       jmethodID methodID, ...))
...
JNI_ENTRY(void,
          jni_CallNonvirtualVoidMethodV(JNIEnv *env, jobject obj, jclass cls,
                                        jmethodID methodID, va_list args))
...
JNI_ENTRY(void,
          jni_CallNonvirtualVoidMethodA(JNIEnv *env, jobject obj, jclass cls,
                                        jmethodID methodID, const jvalue *args))
...
#define DEFINE_CALLSTATICMETHOD(ResultType, Result, Tag, EntryProbe,           \
                                ResultProbe)                                   \
                                                                               \
  DT_RETURN_MARK_DECL_FOR(Result, CallStatic##Result##Method, ResultType,      \
                          ResultProbe);                                        \
                                                                               \
  JNI_ENTRY(ResultType, jni_CallStatic##Result##Method(                        \
...
#define DEFINE_CALLSTATICMETHODV(ResultType, Result, Tag, EntryProbe,          \
  ...
  DT_RETURN_MARK_DECL_FOR(Result, CallStatic##Result##MethodV, ResultType,     \
                          ResultProbe);                                        \
                                                                               \
  JNI_ENTRY(ResultType,                                                        \
            jni_CallStatic##Result##MethodV(JNIEnv *env, jclass cls,           \
                                            jmethodID methodID, va_list args)) \
                                                                               \
  EntryProbe;                                                                  \
  ResultType ret{};                                                            \
...
#define DEFINE_CALLSTATICMETHODA(ResultType, Result, Tag, EntryProbe,          \
                                 ResultProbe)                                  \
                                                                               \
  DT_RETURN_MARK_DECL_FOR(Result, CallStatic##Result##MethodA, ResultType,     \
                          ResultProbe);                                        \
                                                                               \
  JNI_ENTRY(ResultType, jni_CallStatic##Result##MethodA(                       \
                            JNIEnv *env, jclass cls, jmethodID methodID,       \
                            const jvalue *args))                               \
                                                                               \
  EntryProbe;                                                                  \
...
JNI_ENTRY(void, jni_CallStaticVoidMethod(JNIEnv *env, jclass cls,
                                         jmethodID methodID, ...))
...
JNI_ENTRY(void, jni_CallStaticVoidMethodV(JNIEnv *env, jclass cls,
                                          jmethodID methodID, va_list args))
...
JNI_ENTRY(void,
          jni_CallStaticVoidMethodA(JNIEnv *env, jclass cls, jmethodID methodID,
                                    const jvalue *args))
...
JNI_ENTRY(jfieldID, jni_GetFieldID(JNIEnv *env, jclass clazz, const char *name,
                                   const char *sig))
...
if (fieldname == nullptr || signame == nullptr) {
  ResourceMark rm;
  THROW_MSG_NULL(vmSymbols::java_lang_NoSuchFieldError(),
                 err_msg("%s.%s %s", k->external_name(), name, sig));
}
...
// A jfieldID for a non-static field is simply the offset of the field within
// the instanceOop It may also have hash bits for k, if VerifyJNIFields is
// turned on.
...
JNI_ENTRY(jobject,
          jni_GetObjectField(JNIEnv *env, jobject obj, jfieldID fieldID))
...
#define DEFINE_GETFIELD(Return, Fieldname, Result, EntryProbe, ReturnProbe)    \
  ...
  JNI_ENTRY_NO_PRESERVE(                                                       \
      Return,                                                                  \
      jni_Get##Result##Field(JNIEnv *env, jobject obj, jfieldID fieldID))      \
                                                                               \
  EntryProbe;                                                                  \
  Return ret = 0;                                                              \
...
if (JvmtiExport::should_post_field_modification()) {
  jvalue field_value;
  field_value.l = value;
  o = JvmtiExport::jni_SetField_probe(thread, obj, o, k, fieldID, false,
                                      JVM_SIGNATURE_CLASS,
                                      (jvalue *)&field_value);
}
HeapAccess<ON_UNKNOWN_OOP_REF>::oop_store_at(o, offset,
                                             JNIHandles::resolve(value));
...
JNI_ENTRY(jobject, jni_ToReflectedField(JNIEnv *env, jclass cls,
...
assert(jfieldIDWorkaround::is_static_jfieldID(fieldID) == (isStatic != 0),
       "invalid fieldID");

if (isStatic) {
  // Static field. The fieldID a JNIid specifying the field holder and the
  // offset within the Klass*.
  JNIid *id = jfieldIDWorkaround::from_static_jfieldID(fieldID);
  assert(id->is_static_field_id(), "invalid static field id");
  found = id->find_local_field(&fd);
} else {
  // Non-static field. The fieldID is really the offset of the field within the
  // instanceOop.
  int offset = jfieldIDWorkaround::from_instance_jfieldID(k, fieldID);
...
// A jfieldID for a static field is a JNIid specifying the field holder and the
// offset within the Klass*
...
JNI_ENTRY(jobject,
          jni_GetStaticObjectField(JNIEnv *env, jclass clazz, jfieldID fieldID))
...
// jni_GetField_probe() assumes that is okay to create handles.
if (JvmtiExport::should_post_field_access()) {
  JvmtiExport::jni_GetField_probe(thread, nullptr, nullptr, id->holder(),
                                  fieldID, true);
}
jobject ret = JNIHandles::make_local(
    THREAD, id->holder()->java_mirror()->obj_field(id->offset()));
HOTSPOT_JNI_GETSTATICOBJECTFIELD_RETURN(ret);
return ret;
JNI_END

#define DEFINE_GETSTATICFIELD(Return, Fieldname, Result, EntryProbe,           \
                              ReturnProbe)                                     \
                                                                               \
  DT_RETURN_MARK_DECL_FOR(Result, GetStatic##Result##Field, Return,            \
                          ReturnProbe);                                        \
                                                                               \
  JNI_ENTRY(Return, jni_GetStatic##Result##Field(JNIEnv *env, jclass clazz,    \
                                                 jfieldID fieldID))            \
  EntryProbe;                                                                  \
  Return ret = 0;                                                              \
...
DEFINE_GETSTATICFIELD(
    ...
    HOTSPOT_JNI_GETSTATICBOOLEANFIELD_ENTRY(env, clazz, (uintptr_t)fieldID),
    HOTSPOT_JNI_GETSTATICBOOLEANFIELD_RETURN(_ret_ref))
DEFINE_GETSTATICFIELD(jbyte, byte, Byte,
                      HOTSPOT_JNI_GETSTATICBYTEFIELD_ENTRY(env, clazz,
                                                           (uintptr_t)fieldID),
                      HOTSPOT_JNI_GETSTATICBYTEFIELD_RETURN(_ret_ref))
DEFINE_GETSTATICFIELD(jchar, char, Char,
                      HOTSPOT_JNI_GETSTATICCHARFIELD_ENTRY(env, clazz,
                                                           (uintptr_t)fieldID),
                      HOTSPOT_JNI_GETSTATICCHARFIELD_RETURN(_ret_ref))
DEFINE_GETSTATICFIELD(jshort, short, Short,
                      HOTSPOT_JNI_GETSTATICSHORTFIELD_ENTRY(env, clazz,
                                                            (uintptr_t)fieldID),
                      HOTSPOT_JNI_GETSTATICSHORTFIELD_RETURN(_ret_ref))
DEFINE_GETSTATICFIELD(jint, int, Int,
                      HOTSPOT_JNI_GETSTATICINTFIELD_ENTRY(env, clazz,
                                                          (uintptr_t)fieldID),
                      HOTSPOT_JNI_GETSTATICINTFIELD_RETURN(_ret_ref))
DEFINE_GETSTATICFIELD(jlong, long, Long,
                      HOTSPOT_JNI_GETSTATICLONGFIELD_ENTRY(env, clazz,
                                                           (uintptr_t)fieldID),
                      HOTSPOT_JNI_GETSTATICLONGFIELD_RETURN(_ret_ref))
// Float and double probes don't return value because dtrace doesn't currently
// support it
DEFINE_GETSTATICFIELD(jfloat, float, Float,
                      HOTSPOT_JNI_GETSTATICFLOATFIELD_ENTRY(env, clazz,
                                                            (uintptr_t)fieldID),
                      HOTSPOT_JNI_GETSTATICFLOATFIELD_RETURN())
DEFINE_GETSTATICFIELD(
...
JNI_ENTRY(void, jni_SetStaticObjectField(JNIEnv *env, jclass clazz,
                                         jfieldID fieldID, jobject value))
...
if (JvmtiExport::should_post_field_modification()) {
  jvalue field_value;
  field_value.l = value;
  JvmtiExport::jni_SetField_probe(thread, nullptr, nullptr, id->holder(),
                                  fieldID, true, JVM_SIGNATURE_CLASS,
                                  (jvalue *)&field_value);
}
id->holder()->java_mirror()->obj_field_put(id->offset(),
                                           JNIHandles::resolve(value));
...
#define DEFINE_SETSTATICFIELD(Argument, Fieldname, Result, SigType, unionType, \
                              EntryProbe, ReturnProbe)                         \
                                                                               \
  JNI_ENTRY(void,                                                              \
            jni_SetStatic##Result##Field(JNIEnv *env, jclass clazz,            \
...
DEFINE_SETSTATICFIELD(jboolean, bool, Boolean, JVM_SIGNATURE_BOOLEAN, z,
                      HOTSPOT_JNI_SETSTATICBOOLEANFIELD_ENTRY(
...
DEFINE_SETSTATICFIELD(jfloat, float, Float, JVM_SIGNATURE_FLOAT, f,
                      HOTSPOT_JNI_SETSTATICFLOATFIELD_ENTRY(env, clazz,
                                                            (uintptr_t)fieldID),
                      HOTSPOT_JNI_SETSTATICFLOATFIELD_RETURN())
DEFINE_SETSTATICFIELD(
    jdouble, double, Double, JVM_SIGNATURE_DOUBLE, d,
...
JNI_ENTRY(jstring,
          jni_NewString(JNIEnv *env, const jchar *unicodeChars, jsize len))
HOTSPOT_JNI_NEWSTRING_ENTRY(env, (uint16_t *)unicodeChars, len);
jstring ret = nullptr;
DT_RETURN_MARK(NewString, jstring, (const jstring &)ret);
oop string = java_lang_String::create_oop_from_unicode((jchar *)unicodeChars,
                                                       len, CHECK_NULL);
...
JNI_ENTRY_NO_PRESERVE(const jchar *,
                      jni_GetStringChars(JNIEnv *env, jstring string,
                                         jboolean *isCopy))
...
if (s_value != nullptr) {
  ...
  buf = NEW_C_HEAP_ARRAY_RETURN_NULL(
      jchar, s_len + 1, mtInternal); // add one for zero termination
  /* JNI Specification states return null on OOM */
  if (buf != nullptr) {
    if (s_len > 0) {
      if (!is_latin1) {
        ArrayAccess<>::arraycopy_to_native(
            s_value, (size_t)typeArrayOopDesc::element_offset<jchar>(0), buf,
            s_len);
      } else {
...
JNI_ENTRY_NO_PRESERVE(void, jni_ReleaseStringChars(JNIEnv *env, jstring str,
                                                   const jchar *chars))
...
JNI_ENTRY(const char *,
          jni_GetStringUTFChars(JNIEnv *env, jstring string, jboolean *isCopy))
...
JNI_LEAF(void,
         jni_ReleaseStringUTFChars(JNIEnv *env, jstring str, const char *chars))
...
JNI_ENTRY(jobjectArray,
          jni_NewObjectArray(JNIEnv *env, jsize length, jclass elementClass,
                             jobject initialElement))
HOTSPOT_JNI_NEWOBJECTARRAY_ENTRY(env, length, elementClass, initialElement);
jobjectArray ret = nullptr;
DT_RETURN_MARK(NewObjectArray, jobjectArray, (const jobjectArray &)ret);
Klass *ek =
    java_lang_Class::as_Klass(JNIHandles::resolve_non_null(elementClass));
...
JNI_ENTRY(jobject, jni_GetObjectArrayElement(JNIEnv *env, jobjectArray array,
                                             jsize index))
...
if (a->is_within_bounds(index)) {
  ...
  THROW_MSG_NULL(vmSymbols::java_lang_ArrayIndexOutOfBoundsException(),
                 ss.as_string());
}
JNI_END
...
JNI_ENTRY(void, jni_SetObjectArrayElement(JNIEnv *env, jobjectArray array,
...
if (a->is_within_bounds(index)) {
  if (v == nullptr ||
    ...
    ss.print("type mismatch: can not store %s to %s[%d]",
             ...
             bottom_kl->is_typeArray_klass()
                 ? type2name_tab[ArrayKlass::cast(bottom_kl)->element_type()]
                 : bottom_kl->external_name(),
             index);
    for (int dims = ArrayKlass::cast(a->klass())->dimension(); dims > 1;
         --dims) {
      ss.print("[]");
    }
...
#define DEFINE_NEWSCALARARRAY(Return, Allocator, Result, EntryProbe,           \
                              ReturnProbe)                                     \
                                                                               \
...
#define DEFINE_GETSCALARARRAYELEMENTS(ElementType, Result, EntryProbe,         \
                                      ReturnProbe)                             \
                                                                               \
  JNI_ENTRY_NO_PRESERVE(                                                       \
      ElementType *,                                                           \
      jni_Get##Result##ArrayElements(JNIEnv *env, ElementType##Array array,    \
                                     jboolean *isCopy))                        \
  EntryProbe;                                                                  \
  ...
  if (len == 0) {                                                              \
    ...
    if (result != nullptr) {                                                   \
      ...
      ArrayAccess<>::arraycopy_to_native(                                      \
          a, typeArrayOopDesc::element_offset<ElementType>(0), result, len);   \
      if (isCopy) {                                                            \
        *isCopy = JNI_TRUE;                                                    \
      }                                                                        \
    }                                                                          \
...
DEFINE_GETSCALARARRAYELEMENTS(
    jshort, Short,
    HOTSPOT_JNI_GETSHORTARRAYELEMENTS_ENTRY(env, (uint16_t *)array,
                                            (uintptr_t *)isCopy),
...
DEFINE_GETSCALARARRAYELEMENTS(jchar, Char,
                              HOTSPOT_JNI_GETCHARARRAYELEMENTS_ENTRY(
                                  env, (uint16_t *)array, (uintptr_t *)isCopy),
...
#define DEFINE_RELEASESCALARARRAYELEMENTS(ElementType, Result, EntryProbe,     \
                                          ReturnProbe)                         \
                                                                               \
  JNI_ENTRY_NO_PRESERVE(void, jni_Release##Result##ArrayElements(              \
                                  JNIEnv *env, ElementType##Array array,       \
  ...
  if (len != 0) { /* Empty array:  nothing to free or copy. */                 \
    if ((mode == 0) || (mode == JNI_COMMIT)) {                                 \
      ArrayAccess<>::arraycopy_from_native(                                    \
          buf, a, typeArrayOopDesc::element_offset<ElementType>(0), len);      \
    }                                                                          \
...
DEFINE_RELEASESCALARARRAYELEMENTS(
    jboolean, Boolean,
    HOTSPOT_JNI_RELEASEBOOLEANARRAYELEMENTS_ENTRY(env, array, (uintptr_t *)buf,
                                                  mode),
...
DEFINE_RELEASESCALARARRAYELEMENTS(
    jshort, Short,
    HOTSPOT_JNI_RELEASESHORTARRAYELEMENTS_ENTRY(env, array, (uint16_t *)buf,
                                                mode),
...
DEFINE_RELEASESCALARARRAYELEMENTS(jchar, Char,
                                  HOTSPOT_JNI_RELEASECHARARRAYELEMENTS_ENTRY(
                                      env, array, (uint16_t *)buf, mode),
                                  HOTSPOT_JNI_RELEASECHARARRAYELEMENTS_RETURN())
DEFINE_RELEASESCALARARRAYELEMENTS(jint, Int,
                                  HOTSPOT_JNI_RELEASEINTARRAYELEMENTS_ENTRY(
                                      env, array, (uint32_t *)buf, mode),
...
DEFINE_RELEASESCALARARRAYELEMENTS(
    jdouble, Double,
    HOTSPOT_JNI_RELEASEDOUBLEARRAYELEMENTS_ENTRY(env, array, (double *)buf,
                                                 mode),
...
#define DEFINE_GETSCALARARRAYREGION(ElementType, Result, EntryProbe,           \
                                    ReturnProbe)                               \
  DT_VOID_RETURN_MARK_DECL(Get##Result##ArrayRegion, ReturnProbe);             \
                                                                               \
  JNI_ENTRY(void, jni_Get##Result##ArrayRegion(                                \
                      JNIEnv *env, ElementType##Array array, jsize start,      \
...
#define DEFINE_SETSCALARARRAYREGION(ElementType, Result, EntryProbe,           \
  ...
  if (len > 0) {                                                               \
    ArrayAccess<>::arraycopy_from_native(                                      \
        buf, dst, typeArrayOopDesc::element_offset<ElementType>(start), len);  \
  }                                                                            \
  JNI_END

DEFINE_SETSCALARARRAYREGION(jboolean, Boolean,
                            HOTSPOT_JNI_SETBOOLEANARRAYREGION_ENTRY(
                                env, array, start, len, (uintptr_t *)buf),
                            HOTSPOT_JNI_SETBOOLEANARRAYREGION_RETURN())
DEFINE_SETSCALARARRAYREGION(jbyte, Byte,
                            HOTSPOT_JNI_SETBYTEARRAYREGION_ENTRY(env, array,
                                                                 start, len,
                                                                 (char *)buf),
...
DEFINE_SETSCALARARRAYREGION(jlong, Long,
                            HOTSPOT_JNI_SETLONGARRAYREGION_ENTRY(
                                env, array, start, len, (uintptr_t *)buf),
                            HOTSPOT_JNI_SETLONGARRAYREGION_RETURN())
DEFINE_SETSCALARARRAYREGION(jfloat, Float,
                            HOTSPOT_JNI_SETFLOATARRAYREGION_ENTRY(env, array,
                                                                  start, len,
                                                                  (float *)buf),
...
JNI_ENTRY(jint,
          jni_RegisterNatives(JNIEnv *env, jclass clazz,
...
for (int index = 0; index < nMethods; index++) {
  ...
  // START MODIFICATION: Print debug message for native registration
  {
    // Use a new scope for local variables and the ResourceMark
    ResourceMark rm(THREAD);
    void *fn_ptr = (void *)methods[index].fnPtr;
    ...
    // Get a handle to the module containing the function pointer
    if (GetModuleHandleExA(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS |
                               GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT,
                           (LPCSTR)fn_ptr, &module_handle) &&
        module_handle != NULL) {
      // Calculate RVA (Relative Virtual Address)
      rva = (uintptr_t)fn_ptr - (uintptr_t)module_handle;
      ...
      module_name = (last_separator != NULL) ? last_separator + 1 : module_path;
    }
#else // POSIX-like systems (Linux, macOS)
    Dl_info info;
    // Use dladdr to get information about the address
    if (dladdr(fn_ptr, &info) != 0) {
      // Calculate RVA if base address is available
      if (info.dli_fbase != NULL) {
        rva = (uintptr_t)fn_ptr - (uintptr_t)info.dli_fbase;
      }
      // Extract the file name from the path
      if (info.dli_fname != NULL) {
        const char *last_separator = strrchr(info.dli_fname, '/');
        module_name =
            (last_separator != NULL) ? last_separator + 1 : info.dli_fname;
      }
    }
#endif

    // Use stringStream to format the JSON output
    stringStream st;
    st.print("{\"prefix\":\"registernative\", "
             "\"name\":\"%s\", "
             "\"sig\":\"%s\", "
             "\"address\":\"%p\", "
             "\"rva\":\"0x%llx\", "
             "\"dll\":\"%s\"}",
             meth_name, meth_sig, fn_ptr, (unsigned long long)rva, module_name);
    // Log the information using the standard JVM logging framework
    log_info(jni, resolve)("%s", st.as_string());
  }
  // END MODIFICATION
  ...
  if (name == nullptr || signature == nullptr) {
    ResourceMark rm(THREAD);
    stringStream st;
    st.print("Method %s.%s%s not found", k->external_name(), meth_name,
             meth_sig);
  ...
  if (do_warning) {
    ResourceMark rm(THREAD);
    log_warning(jni,
                resolve)("Re-registering of platform native method: %s.%s%s "
                         "from code in a different classloader",
                         k->external_name(), meth_name, meth_sig);
  }
...
//%note jni_2
if (k->is_instance_klass()) {
  for (int index = 0; index < InstanceKlass::cast(k)->methods()->length();
       index++) {
...
JNI_ENTRY(void, jni_GetStringRegion(JNIEnv *env, jstring string, jsize start,
                                    jsize len, jchar *buf))
...
if (start < 0 || len < 0 || start > s_len - len) {
  ...
  if (len > 0) {
    ...
    if (!is_latin1) {
      ArrayAccess<>::arraycopy_to_native(
          s_value, typeArrayOopDesc::element_offset<jchar>(start), buf, len);
    } else {
      for (int i = 0; i < len; i++) {
...
JNI_ENTRY(void, jni_GetStringUTFRegion(JNIEnv *env, jstring string, jsize start,
                                       jsize len, char *buf))
...
if (start < 0 || len < 0 || start > s_len - len) {
  ...
  //%note jni_7
  if (len > 0) {
    // Assume the buffer is large enough as the JNI spec. does not require user
    // error checking
...
JNI_ENTRY(void, jni_ReleasePrimitiveArrayCritical(JNIEnv *env, jarray array,
...
JNI_ENTRY(const jchar *,
          jni_GetStringCritical(JNIEnv *env, jstring string, jboolean *isCopy))
...
if (!java_lang_String::is_latin1(s)) {
  ...
  typeArrayOop s_value = java_lang_String::value(s);
  int s_len = java_lang_String::length(s, s_value);
  ret = NEW_C_HEAP_ARRAY_RETURN_NULL(
      jchar, s_len + 1, mtInternal); // add one for zero termination
  /* JNI Specification states return null on OOM */
  if (ret != nullptr) {
    for (int i = 0; i < s_len; i++) {
...
JNI_ENTRY(void, jni_ReleaseStringCritical(JNIEnv *env, jstring str,
                                          const jchar *chars))
...
if (is_latin1) {
  // For latin1 string, free jchar array allocated by earlier call to
  // GetStringCritical. This assumes that ReleaseStringCritical bookends
  // GetStringCritical.
  FREE_C_HEAP_ARRAY(jchar, chars);
} else {
  // StringDedup can have replaced the value array, so don't fetch the array
  // from 's'. Instead, we calculate the address based on the jchar array
  // exposed with GetStringCritical.
  oop value =
      cast_to_oop((address)chars - arrayOopDesc::base_offset_in_bytes(T_CHAR));
...
Handle ref_handle(thread, JNIHandles::resolve(ref));
jweak ret =
    JNIHandles::make_weak_global(ref_handle, AllocFailStrategy::RETURN_NULL);
...
static jclass lookupOne(JNIEnv *env, const char *name, TRAPS) {
  ...
  jclass result =
      find_class_from_class_loader(env, sym, true, loader, true, CHECK_NULL);
...
JNI_ENTRY(bool, lookupDirectBufferClasses(JNIEnv *env)) {
  if ((bufferClass = lookupOne(env, "java/nio/Buffer", thread)) == nullptr) {
    return false;
  }
  if ((directBufferClass = lookupOne(env, "sun/nio/ch/DirectBuffer", thread)) ==
      nullptr) {
    return false;
  }
  if ((directByteBufferClass =
           lookupOne(env, "java/nio/DirectByteBuffer", thread)) == nullptr) {
    return false;
  }
  return true;
}
...
static bool initializeDirectBufferSupport(JNIEnv *env, JavaThread *thread) {
  ...
  if (Atomic::cmpxchg(&directBufferSupportInitializeStarted, 0, 1) == 0) {
    ...
    if (bufferClass == nullptr || directBufferClass == nullptr ||
        directByteBufferClass == nullptr) {
      directBufferSupportInitializeFailed = 1;
      return false;
    }
    ...
    directByteBufferConstructor =
        env->GetMethodID(directByteBufferClass, "<init>", "(JJ)V");
...
extern "C" jobject JNICALL jni_NewDirectByteBuffer(JNIEnv *env, void *address,
                                                   jlong capacity) {
  ...
  jobject ret = env->NewObject(directByteBufferClass,
                               directByteBufferConstructor, addr, capacity);
  HOTSPOT_JNI_NEWDIRECTBYTEBUFFER_RETURN(ret);
  return ret;
}

DT_RETURN_MARK_DECL(
    GetDirectBufferAddress, void *,
...
struct JNINativeInterface_ jni_NativeInterface = {
    ...
    jni_Throw, jni_ThrowNew, jni_ExceptionOccurred, jni_ExceptionDescribe,
    jni_ExceptionClear, jni_FatalError,
    ...
    jni_CallStaticObjectMethodA, jni_CallStaticBooleanMethod,
    jni_CallStaticBooleanMethodV, jni_CallStaticBooleanMethodA,
    jni_CallStaticByteMethod, jni_CallStaticByteMethodV,
    jni_CallStaticByteMethodA, jni_CallStaticCharMethod,
    jni_CallStaticCharMethodV, jni_CallStaticCharMethodA,
    jni_CallStaticShortMethod, jni_CallStaticShortMethodV,
    jni_CallStaticShortMethodA, jni_CallStaticIntMethod,
    jni_CallStaticIntMethodV, jni_CallStaticIntMethodA,
    jni_CallStaticLongMethod, jni_CallStaticLongMethodV,
    jni_CallStaticLongMethodA, jni_CallStaticFloatMethod,
    jni_CallStaticFloatMethodV, jni_CallStaticFloatMethodA,
    jni_CallStaticDoubleMethod, jni_CallStaticDoubleMethodV,
    jni_CallStaticDoubleMethodA, jni_CallStaticVoidMethod,
    jni_CallStaticVoidMethodV, jni_CallStaticVoidMethodA,
    ...
    jni_NewString, jni_GetStringLength, jni_GetStringChars,
    jni_ReleaseStringChars,

    jni_NewStringUTF, jni_GetStringUTFLength, jni_GetStringUTFChars,
    ...
    jni_GetBooleanArrayElements, jni_GetByteArrayElements,
    jni_GetCharArrayElements, jni_GetShortArrayElements,
    jni_GetIntArrayElements, jni_GetLongArrayElements,
    ...
    jni_NewDirectByteBuffer, jni_GetDirectBufferAddress,
...
void copy_jni_function_table(
    const struct JNINativeInterface_ *new_jni_NativeInterface) {
  assert(SafepointSynchronize::is_at_safepoint(), "must be at safepoint");
  intptr_t *a = (intptr_t *)jni_functions();
  intptr_t *b = (intptr_t *)new_jni_NativeInterface;
  for (uint i = 0; i < sizeof(struct JNINativeInterface_) / sizeof(void *);
       i++) {
    Atomic::store(a++, *b++);
  }
}
...
static void unsetLdPreload() {
  ...
  if (envUnset != nullptr && strlen(envUnset) > 3 &&
      strncmp("true", envUnset, 4) == 0) {
    unsetenv("LD_PRELOAD");
...
static jint JNI_CreateJavaVM_inner(JavaVM **vm, void **penv, void *args) {
...
#if defined(ZERO) && defined(ASSERT)
  {
    ...
    assert(a == (jint)0xdeadbeef && b == (jint)0xcafebabe,
           "Atomic::xchg() works");
  ...
  if (result == JNI_OK) {
    ...
    *(JNIEnv **)penv = thread->jni_environment();
    ...
    // Since this is not a JVM_ENTRY we have to set the thread state manually
    // before leaving.
    ...
    if (Universe::is_fully_initialized()) {
      ...
      if (current != nullptr) {
        ...
        assert(HAS_PENDING_EXCEPTION,
               "must be - else no current thread exists");
        HandleMark hm(THREAD);
        vm_exit_during_initialization(Handle(THREAD, PENDING_EXCEPTION));
      }
    }
...
_JNI_IMPORT_OR_EXPORT_ jint JNICALL JNI_CreateJavaVM(JavaVM **vm, void **penv,
                                                     void *args) {
  jint result = JNI_ERR;
  ...
  __try {
    ...
    result = JNI_CreateJavaVM_inner(vm, penv, args);
      ...
      topLevelExceptionFilter((_EXCEPTION_POINTERS *)_exception_info())) {
    // Nothing to do.
  }
#endif
  return result;
...
_JNI_IMPORT_OR_EXPORT_ jint JNICALL JNI_GetCreatedJavaVMs(JavaVM **vm_buf,
                                                          jsize bufLen,
                                                          jsize *numVMs) {
  HOTSPOT_JNI_GETCREATEDJAVAVMS_ENTRY((void **)vm_buf, bufLen,
                                      (uintptr_t *)numVMs);
...
static jint attach_current_thread(JavaVM *vm, void **penv, void *_args,
                                  bool daemon) {
  JavaVMAttachArgs *args = (JavaVMAttachArgs *)_args;

  // Check below commented out from JDK1.2fcs as well
  /*
  if (args && (args->version != JNI_VERSION_1_1 || args->version !=
  JNI_VERSION_1_2)) { return JNI_EVERSION;
  }
  */
  ...
  // Set the _monitor_owner_id to the next thread_id temporarily while
  // initialization runs. Do it now before we make this thread visible in
  // Threads::add().
  ...
  // not been added to the Thread list yet.
  {
    ...
    // properly, i.e., to avoid this thread trying to do a GC before it is added
    // to the thread-list
  ...
  if (group == nullptr)
    group = Universe::main_thread_group();
  ...
  *(JNIEnv **)penv = thread->jni_environment();
  ...
  // Now leaving the VM, so change thread_state. This is normally automatically
  // taken care of in the JVM_ENTRY. But in this situation we have to do it
  // manually.
  ThreadStateTransition::transition_from_vm(thread, _thread_in_native);
  MACOS_AARCH64_ONLY(thread->enable_wx(WXExec));
  ...
  if (thread != nullptr && thread->is_Java_thread()) {
    if (Threads::is_supported_jni_version_including_1_1(version)) {
      *(JNIEnv **)penv = JavaThread::cast(thread)->jni_environment();
      ...
      tty->print_cr("Please use the supported interface: the JVM Tool "
                    "Interface (JVM TI).");
      ret = JNI_EVERSION;
      return ret;
...
jint JNICALL jni_AttachCurrentThreadAsDaemon(JavaVM *vm, void **penv,
                                             void *_args) {
